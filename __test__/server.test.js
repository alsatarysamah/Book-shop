...
let book ={testbook:{name:"book1",author:"samah"}};
let ubook ={testbook:{name:"book2",author:"samah"}};
...
describe("Auth Router", () => {
  ...
  it("POST Succeeds with a valid token CREAT", async () => {
    ...
    const bearerResponse = await mockRequest
      .post("/book")
      .set("Authorization", `Bearer ${accessToken}`).send(book.testbook);
  ...
  it("Update Succeeds with a valid token CREAT", async () => {
    ...
    .post("/book")
    .set("Authorization", `Bearer ${accessToken}`).send(book.testbook);
    const book2 = await mockRequest
      .put("/book/1")
      .set("Authorization", `Bearer ${accessToken}`).send(ubook.testbook);
    if (response.body.user.actions.length >= 3) {
      expect(book2.status).toBe(201);
    } else {
      expect(book2.status).toBe(500);
    }
  });
  ////////////////////////////////////////////////////
